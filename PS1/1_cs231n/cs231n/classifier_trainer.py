  ...
  def train(self, X, y, X_val, y_val, 
    ...
    for it in range(num_iters):
      ...
      for p in model:
        ...
        if update == 'sgd':
          dx = -learning_rate * grads[p].reshape(model[p].shape)
        elif update == 'momentum':
          if not p in self.step_cache: 
            self.step_cache[p] = np.zeros(grads[p].reshape(model[p].shape).shape)
          ...
          #####################################################################
          self.step_cache[p] = self.step_cache[p]*momentum + learning_rate*grads[p].reshape(model[p].shape)
          dx = -self.step_cache[p].reshape(model[p].shape)
          #####################################################################
          #                      END OF YOUR CODE                             #
          #####################################################################
        elif update == 'rmsprop':
          ...
          #####################################################################
          self.step_cache[p] = decay_rate * self.step_cache[p] + (1 - decay_rate) * grads[p].reshape(model[p].shape)*grads[p].reshape(model[p].shape)
          dx = - learning_rate * grads[p].reshape(model[p].shape) / np.sqrt(self.step_cache[p] + 1e-8)
          #####################################################################
          #                      END OF YOUR CODE                             #
          #####################################################################
        else:
